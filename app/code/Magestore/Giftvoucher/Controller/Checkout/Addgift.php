    ...
    public function execute()
    {
        ...
        if ($quote->getCouponCode() && !$helper->getGeneralConfig('use_with_coupon')
            ...
            if (!count($addCodes)) {
                ...
                if ($max) {
                    $errorMessage .= __('You have %1 time(s) remaining to re-enter Gift Card code.',
                        $max - count($codes));
                }
            ...
            foreach ($addCodes as $code) {
                ...
                elseif (!$giftVoucher->validate($quote->setQuote($quote))) {
                    $result['error'] = __('You can’t use this gift code since its conditions haven’t been met. Please check these conditions at ' . $this->_url->getUrl('giftvoucher/index/check'));
                }
                    ...
                    if ($helper->canUseCode($giftVoucher)) {
                        ...
                        if ($updatepayment xor ($quote->getGrandTotal() < 0.001)) {
                            ...
                            if ($giftVoucher->getCustomerId() == $this->getCusomterSessionModel()->getCustomerId()
                                ...
                                $result['notice'] .= __('Please note that gift code "%1" has been sent to your friend. When using, both you and your friend will share the same balance in the gift code.',
