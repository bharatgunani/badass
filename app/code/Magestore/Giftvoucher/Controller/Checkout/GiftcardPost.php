...
class GiftcardPost extends \Magestore\Giftvoucher\Controller\Action
{
    public function execute()
    {
        ...
        $credit          = $this->_objectManager->create('Magestore\Giftvoucher\Model\Credit')->load($customerId,
            'customer_id');
                        ...
                        if ($helper->isAvailableToAddCode()) {
                            foreach ($addcodes as $code) {
                                $giftVoucher = $this->_objectManager->create('Magestore\Giftvoucher\Model\Giftvoucher')
                                    ->loadByCode($code);
                                if (!$helper->checkStore($giftVoucher)) {
                                    $this->messageManager->addError(__('You cannot use this gift code in current store.'));
                                ...
                                if (!$giftVoucher->getId() || $giftVoucher->getSetId()) {
                                    ...
                                    if ($max - count($codes)) {
                                        $this->messageManager->addError(__('You have %1 time(s) remaining to re-enter your Gift Card code.',
                                            $max - count($codes)));
                                    }
                                    ...
                                    if ($helper->canUseCode($giftVoucher)) {
                                        ...
                                        if ($giftVoucher->getCustomerId() && $giftVoucher->getRecipientEmail()
                                            ...
                                            $this->messageManager->addNotice(__('Please note that gift code "%1" has been sent to your friend. When using, both you and your friend will share the same balance in the gift code.',
                                                $helper->getHiddenCode($code)));
                                        }
                                        $quote->getShippingAddress()->setCollectShippingRates(true);
                                        $quote->setTotalsCollectedFlag(false)->collectTotals();
                                        if ($flag && $quote->getData('gift_voucher_discount')) {
                                            $isGiftVoucher = true;
                                            foreach ($quote->getAllItems() as $item) {
                                            ...
                                            if (!$isGiftVoucher) {
                                                $this->messageManager->addSuccess(__('Gift code "%1" has been applied successfully.',
                                                    $helper->getHiddenCode($code)));
                                            } else {
                                    ...
                                    $this->messageManager->addError(__('Gift code "%1" is no longer available to use.',
                                        $code));
                                }
                            }
                        } else {
                            $this->messageManager->addError(__('The maximum number of times to enter gift codes is %1!',
                                $max));
                        }
                    } else {
