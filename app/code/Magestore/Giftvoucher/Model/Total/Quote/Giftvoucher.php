        ...
        if ($codes = $session->getGiftCodes()) {
            ...
            foreach ($codesArray as $key => $code) {
                ...
                if ($model->getStatus() != \Magestore\Giftvoucher\Model\Status::STATUS_ACTIVE
                    ...
                    || !$this->_helperData->checkStore($model)
                ) {
                    $codesBaseDiscount[] = 0;
                    $codesDiscount[] = 0;
                } else {
                    ...
                    $discount = $baseDiscount; //$this->_priceCurrency->convert($baseDiscount);
            ...
            $total->setGiftVoucherDiscount($this->_priceCurrency->convert($totalDiscount));
            ...
            $total->addTotalAmount('giftvoucher', -($this->_priceCurrency->convert($totalDiscount)));
            $total->addBaseTotalAmount('giftvoucher', -$baseTotalDiscount);
            //update quote
            $quote->setBaseGiftVoucherDiscount($baseTotalDiscount);
            $quote->setGiftVoucherDiscount($this->_priceCurrency->convert($totalDiscount));
