...
class Giftvoucher extends \Magento\Rule\Model\AbstractModel
{
    ...
    public function load($id, $field = null)
    {
        ...
        $expiredAt = date(
            "Y-m-d H:i:s",
            $this->_objectManager->create('Magento\Framework\Stdlib\DateTime\DateTime')->timestamp($this->getExpiredAt())
        );
    ...
    public function beforeSave()
    {
        ...
        if ($this->getStatus() == \Magestore\Giftvoucher\Model\Status::STATUS_EXPIRED
            && $expiredAt && $expiredAt > date('Y-m-d')
        ) {
        ...
        if ($expiredAt && $expiredAt < date('Y-m-d')) {
            $this->setStatus(\Magestore\Giftvoucher\Model\Status::STATUS_EXPIRED);
        }
            ...
            try {
                $transport = $this->_transportBuilder->setTemplateIdentifier(
                    ...
                    [
                        'store' => $store,
                        'giftvoucher' => $this,
                        'balance' => $this->_pricingHelper->currency($this->getData('balance'), true, false),
    ...
    public function getPrintTemplate()
    {
        ...
        if (isset($images[0]) && file_exists($images[0])) {
            ...
            //    ->getAbsolutePath('giftvoucher/draw/' . $this->getGiftCode() . '/');
            $replace = $this->_helperData->getStoreManager()->getStore()->getBaseUrl(\Magento\Framework\UrlInterface::URL_TYPE_MEDIA)
                . 'giftvoucher/draw/' . $this->getGiftCode() . '/';
