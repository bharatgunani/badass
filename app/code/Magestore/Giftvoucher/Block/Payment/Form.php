...
//                        $discount['value'] = $this->priceCurrency->convert(abs(round($codesDiscountArray[$key], 2)));
                        $discount['value'] = abs(round($codesDiscountArray[$key], 2));
                    }
                    $discount['code']       = $value;
                    $discount['hiddenCode'] = $this->_helperData->getHiddenCode($value);
                    $discount['discount']   = $this->priceCurrency->format(abs(round($codesDiscountArray[$key], 2)), false);
