...
class Payment implements \Magento\Framework\Option\ArrayInterface
{
    ...
    public function __construct(
        ...
        $this->_allowPayments = array('cashforpos', 'codforpos', 'ccforpos',
                                      'cp1forpos', 'cp2forpos', 'paypal_direct', 'authorizenet_directpost', 'cryozonic_stripe');
        $this->_ccPayments = array('authorizenet_directpost', 'cryozonic_stripe');
    }
    ...
    public function getOptionArray()
    {
        ...
        if (count($collection) > 0) {
            foreach ($collection as $item) {
                if (!in_array($item->getCode(), $this->_allowPayments))
                    continue;
                if (!$this->_paymentHelper->isAllowOnWebPOS($item->getCode()))
                    continue;
                $title = $item->getTitle() ? $item->getTitle() : $item->getCode();
                $options["'".$item->getCode()."'"] = $title;
            }
        }
        return $options;
    }
