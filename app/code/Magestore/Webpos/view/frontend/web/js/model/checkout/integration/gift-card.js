        ...
        return modelAbstract.extend({
            ...
            updateBalance: function(giftcardCode){
                ...
                if(giftcardCode){
                    ...
                    deferred.done(function (response) {
                        var data =  JSON.parse(response);
                        if(typeof data.base_balance != 'undefined' && typeof data.base_balance != undefined && data.base_balance){
            ...
            applyGiftCode: function(code, value){
                var appliedCard = ko.utils.arrayFirst(this.appliedCards(), function(giftcard) {
                    return (giftcard.code == code);
                });
                if(appliedCard){
                    if(value > 0) {
                        appliedCard.value(value);
                        appliedCard.balance(this.balance());
                        appliedCard.remain(this.balanceAfterApply());
                        appliedCard.usemax(this.useMaxPoint());
                    }else{
                        this.appliedCards.remove(appliedCard);
                        if(appliedCard.code == this.giftcardCode()){
                            this.resetEditingCard();
                            if(this.appliedCards().length > 0){
                                this.editCard(this.appliedCards()[this.appliedCards().length - 1]);
                            }
                        }
                    }
                }else{
                    if(value > 0){
                        this.appliedCards.push({
                            code:code,
                            value:ko.observable(value),
                            balance:ko.observable(this.balance()),
                            remain:ko.observable(this.balanceAfterApply()),
                            usemax:ko.observable(this.useMaxPoint())
                        });
                    }
                }
                if(this.appliedCards().length == 0){
