    ...
    function ($, ko, ViewManager, utils, Component, CheckoutModel, CartModel, Helper, AutoCheckPromotion, TotalsFactory) {
        ...
        return Component.extend({
            ...
            cartTotal: ko.pureComputed(function(){
                return Helper.convertAndFormatPrice((TotalsFactory.get().grandTotal()) ? TotalsFactory.get().grandTotal():0);
            }),
            ...
            placeOrder: function(){
                ...
                if((!CheckoutModel.selectedPayments() || CheckoutModel.selectedPayments().length <= 0)
                   && TotalsFactory.get().grandTotal() > 0){
                    Helper.alert({
                        priority: "danger",
                        title: "Message",
            ...
            showPayments: ko.pureComputed(function(){
                if(TotalsFactory.get().grandTotal() > 0 || TotalsFactory.get().hasSpecialDiscount() == true)
                    return true;
                return false;
            })
        });
