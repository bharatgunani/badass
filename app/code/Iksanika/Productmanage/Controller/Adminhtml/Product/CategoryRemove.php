    ...
    public function execute()
    {
        ...
        if(is_array($productIds))
        {
            try {
                ...
                $this->messageManager->addSuccess(__('A total of %1 product(s) have been updated.', count($productIds)));
                if(count($categoryNotFounded))
                {
                    $categoryNotFoundedStr = implode(', ', $categoryNotFounded);
                    $this->_getSession()->addError($this->__('A category with index %1 doesn not exist, please, check if category ID was speicifed correctly.'), $categoryNotFoundedStr);
                }
